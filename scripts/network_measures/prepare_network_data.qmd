...
# One option is determine who has less than 25 missing values for the names of 
# alter fields
...
test <- data %>%
  filter(record_id %in% n_unique_alter$record_id)

# Yields about 13 people who have questionable data at baseline 0mo

# Questionable record ids at time 0mo
ids <- c(53, 287, 406, 235, 720, 1016, 1115, 1095, 235, 184, 104, 1058)
...
<!-- ///////////////////// Node/Vertex Betweenness Centrality /////////////////////-->
```{r}
# Betweeness centrality (not the same as centralization)
# graphs dos not include the ego, each graph is comprised of 25 elements, 
...
node_centrality_L <- map2(graphs, ego_ids, ~ {
  ...
  # get the propotion of smokers
  prop <- df %>% slice_max(value) %>% summarise(prop = mean(alt_smoke)) %>% pull(prop)
...
# Create a data frame that is mergeable to main dataset
prop_smoke_node_centrality <- enframe(
  map_dbl(node_centrality_L, ~ {.x$prop_smoker}),
  name = "ego_id",
  value = "prop_smoke_node_centrality"
)
```

```{r}
# for each element in the list,
# if the ties is not null
# then get the data of the ids in the df
# then calculate the proportion of smokers

# else it's just 100%
```

```{r}
# Create a data set of the ego_id, max value for node_centrality, and the number of
# ties encountered in the ego's network.

node_centr_df <- left_join(
# The max value for all egos
map_df(node_centrality_L, ~ .x[[1]][1, ]) %>%
  select(ego_id, value) %>%
  rename(max_node_centr = value),


# The number of ties for all egos
n_tied_node_centrality %>%
  rename(n_tide_node_centr = n_tied),
...
comm_lu_df <- map2_dfr(graphs, ego_ids, ~ {
  ...
  # Create a list that will be be the output
  data.frame(ego_id = .y, len_comm_lu = comm_len, modularity_lu = modularity_score)
...
comm_gn_df <- map2_dfr(graphs, ego_ids, ~ {
  ...
  # Create a list that will be be the output
  data.frame(ego_id = .y, len_comm_gn = comm_len, modularity_gn = modularity_score)
...
comm_infomap_df <- map2_dfr(graphs, ego_ids, ~ {
  ...
  # Create a list that will be be the output
  data.frame(ego_id = .y, len_comm_infomap = comm_len, modularity_infomap = modularity_score)
...
#| eval: false
output_df <- reduce(
  list(node_centr_df, degree_df, closeness_df, comm_lu_df, comm_gn_df, comm_infomap_df),
  left_join, by = "ego_id"
)

write_csv(output_df, "C:\\Users\\rodrica2\\OneDrive - The University of Colorado Denver\\Documents\\DFM\\projects\\hsq\\scripts\\network_measures\\data\\qa\\hsq_node_level_networks_qa.csv")
```
