---
title: "Network conference measures 2025"
date: "`r Sys.Date()`"
format:
  html:
...
```{r, eval = FALSE} 
# Camille's legacy code *** DO NOT RUN ***
# Parameters
...
#   pna2 <- pna %>%
#     select(record_id, alter1:baseline_complete) %>%
#     rowwise() %>%
#     # mutate(var1 = sum(is.na(across(all_of(namerange)))) == 0)
...
#   # This will produce a warning, but it is expected
...
#   # This is a filter to address the warning above
#   alter_filter2 <- et %>%
#     as_tibble() %>%
#     arrange(.egoID) %>%
#     # select(eval(substitute(a1)):eval(substitute(a25))) %>%
#     select(!!sym(a1):!!sym(a25)) %>% # Updated version of selecting
#     # mutate(across(.fns = ~ !is.na(.))) %>% # *** This line triggers a deprecation warning
...
percent_support_quit <- all_data %>%
  ...
  mutate(perc_supp_quit = (row_sum / 25) * 100) %>%
...
network_metrics %>%
  select(-.egoID, -record_id) %>% # Something is wrong with density
  tbl_summary(
    by = arm,
    stat = list(
      density ~ "{mean} ({sd})",
      homophilly_gender ~ "{mean} ({sd})",
      homophilly_age_cat ~ "{mean} ({sd})"    
    ),
    label = list(
      density = "Density",
      ...
      homophilly_gender = "Homophilly (gender)",
      homophilly_age_cat = "Homophilly (age)"
    )
  ) %>%
  add_overall()
```
