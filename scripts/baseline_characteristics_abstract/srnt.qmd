...
# RedCap Participant Management Measures
```{r}
...
njh %>%
  filter(`SI 5a` == "Yes") %>%
  select(arm, "Cigarettes Per Day (SI 7a)") %>%
  tbl_summary(arm,
      statistic = list(all_continuous() ~ "{mean} ({sd}), [{min}; {median}; {max}]"),
...
njh %>%
  filter(`SI 5b` == "Yes") %>%
  select(arm, "Cigars Per Week (SI 7b)") %>%
  tbl_summary(arm,
      digits = list(all_categorical() ~ c(0,1)))
```
...
njh %>%
  filter(`SI 5c` == "Yes") %>%
  select(arm, "Pipes Per Week (SI 7c)") %>%
  tbl_summary(arm)
```
...
njh %>%
  filter(`SI 5d` == "Yes") %>%
  select(arm, "Pouch Per Week (SI 7d)") %>%
  tbl_summary(
    arm,
    digits = list(all_categorical() ~ c(0,1)))
```
