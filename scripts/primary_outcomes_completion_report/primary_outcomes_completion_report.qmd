...
# Finally, this report will export the data to a .csv file for compiling with
# other sources of data such as network measures, and time to event outcomes.
################################################################################
```
...
# Flatten the list
# This produces 941 rows for some reason
flat_lst <- bind_rows(
  data_lst[[1]],
  data_lst[[2]],
  data_lst[[3]],
...
# filter to only the hsqids in the sms data because data contains all hsq ids 
# in the redcap database, but not all were enrolled.
flat_lst %<>%
  filter(hsqid %in% t_to_relapse$hsqid)

# Create a single collapsed variable for Race from the select all that apply options
flat_lst %<>% 
  mutate(across(race___0:race____1, ~ ifelse(.x == "Checked", 1, 0))) %>%
  rowwise() %>%
...
# Score the Self Efficacy 1:5 items
flat_lst %<>%
  ...
  rowwise() %>%
  mutate(se_confidence = sum(c_across(se1:se5), na.rm = TRUE) / 5) %>%
  ungroup() 


flat_lst %>%
  select(-record_id) %>%
  select(hsqid, arm, event_name, everything()) %>%
